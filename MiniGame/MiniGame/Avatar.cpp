...
void Avatar::UpdateVerticalVelocity(float elapsedsec, const Level* level, const Uint8* pKeysState)
{
	...
	if ((pKeysState[SDL_SCANCODE_SPACE] || pKeysState[SDL_SCANCODE_W] || pKeysState[SDL_SCANCODE_UP]) && level->IsOnGround(m_Shape, m_Velocity))
	{
...
{
	return Rectf {m_ClipWidth * m_AnimFrame, m_ClipHeight * (1 + ((int)m_ActionState) + (3 * m_Power)),  m_ClipWidth, m_ClipHeight};
}
