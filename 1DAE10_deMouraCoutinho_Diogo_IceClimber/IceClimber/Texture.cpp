...
Texture::Texture(const std::string& imagePath, float scale)
	:m_Id{ }
	, m_Width{ 10.0f }
	, m_Height{ 10.0f }
	, m_CreationOk{ false }
	, m_Scale{ scale}
{
	CreateFromImage(imagePath, scale);
}
...
void Texture::CreateFromImage(const std::string& path, float scale)
{
	m_CreationOk = true;
...
void Texture::CreateFromSurface(SDL_Surface* pSurface, float scale)
{
	m_CreationOk = true;

	//Get image dimensions
	m_Width = float(pSurface->w) * scale;
	m_Height = float(pSurface->h) * scale;
