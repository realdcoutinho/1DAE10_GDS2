...
bool utils::Raycast(const std::vector<std::vector<Point2f>> vertices, const Point2f& rayP1, const Point2f& rayP2, HitInfo& hitInfo)
{
...
Vector2f utils::Add(Vector2f& vectorA, Vector2f& vectorB)
{
	return Vector2f{ vectorA.x + vectorB.x, vectorA.y + vectorB.y };
}

Vector2f utils::Subtract(Vector2f& vectorA, Vector2f& vectorB)
{
	return Vector2f{ vectorA.x - vectorB.x, vectorA.y - vectorB.y };
}

float utils::DotProduct(Vector2f& vectorA, Vector2f& vectorB)
{
	return (vectorA.x * vectorB.x) + (vectorA.y * vectorB.y);
}

float utils::CrossProduct(Vector2f& vectorA, Vector2f& vectorB)
{
	return (vectorA.x * vectorB.y) - (vectorA.y * vectorB.x);
}

std::string utils::ToString(Vector2f& vector)
{
	return "[" + std::to_string(vector.x) + ", " + std::to_string(vector.y) + "]";
}

Vector2f utils::Scale(Vector2f& vectorA, float scalar)
{
...
Vector2f utils::Normalise(Vector2f& vector)
{
	float length = Length(vector);
	return Vector2f{ vector.x / length, vector.y / length };
}

float utils::AngleBetween(Vector2f& vectorA, Vector2f& vectorB)
{
	return atan2(CrossProduct(vectorA, vectorB), DotProduct(vectorA, vectorB));
}

bool utils::AreEqual(Vector2f& vectorA, Vector2f& vectorB)
{
...
float utils::ConvertDegreesToRadians(float degrees)
{
	return degrees * (utils::g_Pi / 180);
}

float utils::ConvertRadiansToDegrees(float radians)
{
	return radians * (180 / utils::g_Pi);
}

Point2f utils::GetCoordinatesFromRadians(float radius, float radians, Point2f offset)
{
...
float utils::GetRandomFloat(float minValue, float maxValue, int precision)
{
