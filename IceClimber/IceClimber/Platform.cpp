...
void Platform::HandleCollision(Rectf& actorShape, Vector2f& actorVelocity)
{
	...
	HitInfo hitInfo{};
	if (Raycast(m_Collision, actorBottomCenter, actorTopCenter, hitInfo) && actorVelocity.y <= 0) //if hit from the top
	{
...
bool Platform::IsOnGround(const Rectf& actorShape, Vector2f& actorVelocity) const
{
	...
	HitInfo hitInfo{};
	return (Raycast(m_Collision, actorBottomCenter, actorTopCenter, hitInfo) && actorVelocity.y <= 0); //returns a value if the player is on top
}
...
bool Platform::IsOnGround(const Rectf& actorShape) const
{
	...
	HitInfo hitInfo{};
	return (Raycast(m_Collision, actorBottomCenter, actorTopCenter, hitInfo));
}
